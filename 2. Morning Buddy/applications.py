...
if missing_keys:
    raise ValueError(f"Error: Missing required API keys in .env file: {', '.join(missing_keys)}")
...
def get_news(topic: str):
    ...
    """
    try:
        url= f"https://newsapi.org/v2/everything?q={topic}&apiKey={newsapi_key}&pageSize=5&sortBy=publishedAt"
        response =requests.get(url)
